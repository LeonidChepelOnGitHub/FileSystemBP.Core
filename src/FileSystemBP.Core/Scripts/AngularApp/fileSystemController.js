angular.module('fileSystemApp').controller('fileSystemController', ['$window', 'fileSystemService', 'cookieService', '$uibModal', function ($window, fileSystemService, cookieService, $uibModal) {
    ...
    fs.open = function (size, parentSelector) {
...
angular.module('fileSystemApp').controller('ModalInstanceCtrl', function ($uibModalInstance, items) {
        var $ctrl = this;
        $ctrl.items = items;
        $ctrl.selected = {
            item: $ctrl.items[0]
...
angular.module('fileSystemApp').component('modalComponent', {
    templateUrl: 'CreateFile.html',
        bindings: {
            resolve: '<',
            close: '&',
